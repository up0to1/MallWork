...
MallWork 采用“中心化控制面 + 站点侧轻量 Adapter/Connector”。新 `MallWork` 仓库是唯一产品运行时；旧 Pi 代码不进入运行依赖。控制面负责身份、Agent、工作流、策略、审批、审计、评测与部署编排，站点 Connector 只负责把通用契约映射为目标平台 API。
...
需求到阶段的完整追踪见 [Pi OpenSpec 需求映射](../migration/pi-openspec需求映射.md)。
