...
- [ ] Record the new repository branch, HEAD, remotes, and short status.
...
- [ ] Record the legacy repository branch, HEAD, remotes, and exact dirty status.
...
- [ ] Capture the running Compose service names, container IDs, image names, creation/start timestamps, health state, and published ports without mutating them.
...
- [ ] Verify the public health endpoints and record their HTTP status/body.
...
- [ ] Confirm the local `tsx` entry point and TypeScript configuration exist. Do not install or upgrade dependencies unless verification cannot start because an already-declared package is missing.

- [ ] Run every verification script explicitly from the legacy repository; do not substitute `npm test`.
...
- [ ] Capture for each script: command, exit code, pass/fail, and a concise failure reason. Store the result later in `docs/migration/pi遗留资产迁移清单.md`.

- [ ] If a test failure exposes a clear source defect, document it as legacy debt; do not fix runtime code in Phase 1.
...
- [ ] Re-run `git status --short` and confirm that the paths match this list. Stop if unexpected files appear until they are classified.

- [ ] Stage only the ten listed paths with an explicit `git add -- <paths...>` command.

- [ ] Review the staged summary and ensure no secrets, `.env`, generated artifacts, or unrelated paths are staged.
...
- [ ] Add front matter with status, date, owners, scope, and link to the approved design spec.
- [ ] Define the vision, target users, and explicit non-goals.
- [ ] Define the two-level operating model:
  - Digital employees as bounded specialist agents.
  - Digital boss as goal, budget, KPI, workflow, approval, and review supervisor.
- [ ] Define the first agent portfolio: consumer concierge, product research, inventory, pricing, SEO/content, and site builder.
- [ ] Define the merchant and consumer value loops, human approval boundaries, safety expectations, and measurable KPIs.
- [ ] State that the digital boss never writes commerce databases directly and that all store actions go through governed adapters.
- [ ] State the product rollout order and what remains intentionally deferred.
- [ ] Validate every material claim against the approved design spec.
...
- [ ] Add a context diagram showing MallWork control plane, digital boss, specialist agents, approval/audit layer, Store Adapter boundary, and multiple independent stores.
- [ ] Define control-plane modules: identity/tenant context, agent runtime, workflow/scheduler, policy/approval, adapter registry, event ingestion, observability, evaluation, and secrets boundary.
- [ ] Define the per-store connector responsibility and explicitly separate it from core MallWork business logic.
- [ ] Document synchronous commands, asynchronous operations, events/webhooks, correlation IDs, idempotency, and retry rules.
- [ ] Document the trust model: least privilege, tenant/store isolation, credential ownership, secret rotation, audit trail, and no direct agent-to-database access.
- [ ] Document deployment topology for the current single Compose environment and the later multi-store ecosystem without changing the current Compose file.
- [ ] Include failure modes and recovery behavior for adapter outage, partial writes, stale inventory, rate limiting, duplicate events, and long-running jobs.
- [ ] Include an architectural decision record section explaining why centralized control plane plus lightweight adapters was selected.
...
- [ ] Define contract status, compatibility policy, semantic versioning rules, and capability negotiation.
- [ ] Define required execution context fields:
  - `tenant_id`, `store_id`, `actor_id`, `actor_type`
  - roles/scopes
  - `correlation_id`, `request_id`, optional `idempotency_key`
- [ ] Define capability groups and representative operations:
  - Catalog
  - Inventory
  - Pricing
  - Orders
  ...
  - Analytics
  - Deployment
- [ ] Mark each operation as read/write, synchronous/asynchronous, approval-sensitive, and required/optional for conformance.
- [ ] Define shared data rules: UTC timestamps, ISO currency, money in integer minor units, stable external IDs, cursor pagination, optimistic version fields, nullable-field semantics, and validation constraints.
- [ ] Define write safety: idempotency keys, expected-version preconditions, dry-run where supported, confirmation tokens, and compensating-action metadata.
- [ ] Define async operation handles and states: accepted, running, succeeded, failed, canceled, and expired.
- [ ] Define event/webhook envelope, ordering assumptions, deduplication identifier, signature verification, replay window, and acknowledgement behavior.
- [ ] Define normalized errors: `unauthorized`, `forbidden`, `not_found`, `conflict`, `rate_limited`, `transient`, `validation`, and `unsupported`.
- [ ] Define credential handling, secret redaction, timeouts, retries, circuit breaking, telemetry, and audit requirements.
- [ ] Define v1 conformance tests and a ZMall reference adapter test matrix.
...
- [ ] Record the legacy repository path, branch, pre-checkpoint HEAD, checkpoint commit, tag, remotes, checkpoint time, and explicit “not pushed” status.
- [ ] Record all ten checkpointed files with tracked/untracked origin, SHA-256 hash captured before checkpoint, purpose, dependencies, verification result, migration disposition, and target phase.
- [ ] Inventory reusable committed materials under:
...
- [ ] Record all seven verification commands and outcomes from Task 2.
- [ ] State the deletion gate exactly: local checkpoint/tag, private remote backup, complete mapping, target features implemented and verified, and explicit user deletion authorization.
...
- [ ] Extract every requirement and scenario from all five source specs.
- [ ] Assign each a stable mapping ID and record source file/heading.
- [ ] Map it to the new control-plane component, responsible agent, Store Adapter capability, current implementation status, target phase, validation method, and notes.
- [ ] Use only evidence-based status values: existing, partial, planned, rejected, or superseded.
- [ ] Explicitly call out Pi-specific assumptions that are superseded by FastAPI/AgentScope, Redis worker queues, Qdrant, and the browser UI.
- [ ] Cross-check the count of mapped requirements/scenarios against the source headings so none are omitted.
...
- [ ] For each phase define goals, prerequisites, deliverables, risks, tests, data migration impact, rollback strategy, and exit criteria.
- [ ] Add a dependency graph and critical path.
- [ ] Identify decisions deferred until evidence exists, especially identity provider, production database, secret manager, scheduler, and deployment provider.
- [ ] State that Phase 9 cannot delete the legacy repository without a fresh explicit user authorization.
...
- [ ] Validate required files and headings with `Test-Path`, `rg --files docs`, and `rg` heading searches.
- [ ] Scan new documents for placeholders and unresolved language:

  ```powershell
  rg -n "TODO|TBD|FIXME|待定|待补|占位" docs/product docs/architecture docs/contracts docs/migration docs/roadmap
  ```

- [ ] Verify terminology is consistent: MallWork product/repository, “Mall Work” only as the display name where intentional, control plane, digital boss, digital employee, Store Adapter, tenant, store, and ZMall.
- [ ] Verify internal links resolve and every roadmap phase referenced by inventory/mapping exists.
- [ ] Verify only documentation files changed in the new repository before every commit.
- [ ] Create coherent commits with explicit staging, for example:
...
- [ ] Never stage `.env`, runtime data, application source, Compose files, or unrelated user changes.
...
- [ ] Re-run the API and frontend health checks from Task 1.
- [ ] Re-run Compose and container inspection from Task 1.
- [ ] Compare each service’s container ID, creation timestamp, and start timestamp with the baseline; all must be unchanged.
- [ ] Confirm the five expected services remain running and Redis remains healthy.
- [ ] Confirm the new repository diff/history for this phase contains documentation only and the working tree is clean.
- [ ] Confirm the legacy repository is clean at the checkpoint tag and remotes are unchanged.
- [ ] Confirm no push was performed and no remote branch/tag was created by this task.
- [ ] Summarize verification results, local checkpoint/tag, documents created, known legacy test failures, and unchanged access addresses.
